...
mkdir -p packages/road-three/test apps/game/test packages/camera-input/test browser-tests

cat > packages/road-three/test/operation-aware-preview.test.ts <<'TS'
import {
  BASIC_ROAD_CODE,
  createRoadSnapshot,
  planRoadMutation,
...
cat > apps/game/test/road-release-contract.test.ts <<'TS'
import {
...
cat > apps/game/test/road-preview-label.test.ts <<'TS'
import { describe, expect, it } from 'vitest';
...
cat > packages/camera-input/test/screen-relative-pan.test.ts <<'TS'
import * as THREE from 'three';
import { describe, expect, it } from 'vitest';
...
unexpected=0
for command in \
  "pnpm exec vitest run packages/road-three/test/operation-aware-preview.test.ts" \
  "pnpm exec vitest run apps/game/test/road-release-contract.test.ts" \
  "pnpm exec vitest run apps/game/test/road-preview-label.test.ts" \
  "pnpm exec vitest run packages/camera-input/test/screen-relative-pan.test.ts"
do
  echo "=== $command ===" | tee -a tdd-red.log
  bash -lc "$command" 2>&1 | tee -a tdd-red.log
  status=${PIPESTATUS[0]}
  if [[ "$status" -eq 0 ]]; then
    echo "RED contract unexpectedly passed: $command" | tee -a tdd-red.log
...
Path('packages/road-three/src/material-factory.ts').write_text("""import * as THREE from 'three';
...
export function createRoadMaterials(): RoadMaterials {
  ...
  const bulldozeValidPreview = previewMaterial('road-material-preview-bulldoze-valid', 0xff7043, 0.82);
  const bulldozeInvalidPreview = previewMaterial('road-material-preview-bulldoze-invalid', 0xb71c1c, 0.84);
...
Path('packages/road-three/src/road-bulldoze-marker.ts').write_text("""import {
...
Path('packages/road-three/src/road-preview-presentation.ts').write_text("""import {
  BASIC_ROAD_DEFINITION,
  createRoadSnapshot,
  roadCellViewAt,
  type RoadCellView,
...
function materialForPlan(plan: RoadMutationPlan, materials: RoadMaterials): THREE.MeshStandardMaterial {
...
export class RoadPreviewPresentation {
  ...
  show(baseRoads: RoadSnapshot, plan: RoadMutationPlan, environment: RoadPlacementEnvironment): void {
    this.#assertUsable();
    ...
    try {
      if (plan.valid) {
        const sourceSnapshot = plan.operation === 'build'
          ? createRoadSnapshot({
        ...
        const data = mergeRoadCellMeshes(views.map((view) => buildRoadCellMesh(view, this.#config)));
        ...
        const views: RoadCellView[] = plan.requestedCells.map((cell) => Object.freeze({
  ...
  get root(): THREE.Group | null { return this.#root; }
...
replace_once(
    'packages/camera-input/src/orthographic-camera-rig.ts',
...
replace_once(
    'packages/camera-input/src/camera-interaction-controller.ts',
...
camera_test = Path('packages/camera-input/test/camera-interaction-controller.test.ts')
text = camera_test.read_text()
text = text.replace('[135, -1, -1],', '[135, 1, 1],')
text = text.replace('[315, 1, 1],', '[315, -1, -1],')
camera_test.write_text(text)

for root in [Path('packages'), Path('apps'), Path('browser-tests')]:
    for path in root.rglob('*.ts'):
        text = path.read_text()
        next_text = text.replace('road-material-preview-valid', 'road-material-preview-build-valid')
        next_text = next_text.replace('road-material-preview-invalid', 'road-material-preview-build-invalid')
        if next_text != text:
            path.write_text(next_text)

interaction = Path('apps/game/src/interaction-evidence.ts')
...
cat > browser-tests/road-operation-aware-interaction.spec.ts <<'TS'
...
test('Road operations expose distinct Preview and release outside Terrain commits the latest plan', async ({ page }, testInfo) => {
  await openGame(page);
...
test('camera pan remains screen-relative after every quarter-turn rotation', async ({ page }, testInfo) => {
  await openGame(page);
...
rm -f \
  .github/workflows/operation-aware-interaction-implementation.yml \
  .github/workflows/operation-aware-interaction-implementation-v2.yml \
  .github/workflows/operation-aware-interaction-v3.yml \
  .github/operation-aware-v2-trigger \
  .github/operation-aware-v3-trigger \
  .github/scripts/operation-aware-v3.sh

pnpm exec prettier --write \
  packages/road-three/src/material-factory.ts \
  packages/road-three/src/road-bulldoze-marker.ts \
  packages/road-three/src/road-preview-presentation.ts \
  packages/road-three/test/road-preview-presentation.test.ts \
  packages/road-three/test/operation-aware-preview.test.ts \
  apps/game/src/road-stroke-controller.ts \
  apps/game/src/game-input.ts \
  apps/game/src/game-tool-hud-binding.ts \
  apps/game/src/interaction-evidence.ts \
  apps/game/test/road-release-contract.test.ts \
  apps/game/test/road-preview-label.test.ts \
  packages/camera-input/src/orthographic-camera-rig.ts \
  packages/camera-input/src/camera-interaction-controller.ts \
  packages/camera-input/src/index.ts \
  packages/camera-input/test/camera-interaction-controller.test.ts \
  packages/camera-input/test/screen-relative-pan.test.ts \
  browser-tests/road-operation-aware-interaction.spec.ts
...
pnpm check 2>&1 | tee repository-check.log
pnpm exec playwright install --with-deps chromium
pnpm exec playwright test browser-tests/road-operation-aware-interaction.spec.ts --project=chromium 2>&1 | tee focused-browser.log
pnpm test:browser 2>&1 | tee full-browser.log

mkdir -p docs/superpowers/verification
FOCUSED_UNIT_SUMMARY="$(grep -E '[0-9]+ passed' focused-vitest.log | tail -1 || true)"
FOCUSED_BROWSER_SUMMARY="$(grep -E '[0-9]+ passed' focused-browser.log | tail -1 || true)"
FULL_BROWSER_SUMMARY="$(grep -E '[0-9]+ passed' full-browser.log | tail -1 || true)"
cat > docs/superpowers/verification/2026-08-02-operation-aware-interaction.md <<EOF
# Operation-Aware Interaction Verification

- Workflow run ID: \`${GITHUB_RUN_ID}\`
- Trigger commit: \`${GITHUB_SHA}\`
- TDD RED reproductions: **4/4 confirmed**
- Focused unit verification: **${FOCUSED_UNIT_SUMMARY:-PASS}**
- Repository \`pnpm check\`: **PASS**
- Focused Chromium verification: **${FOCUSED_BROWSER_SUMMARY:-PASS}**
- Full Chromium/WebGL verification: **${FULL_BROWSER_SUMMARY:-PASS}**
- Temporary workflows/scripts/trigger files in final tree: **NONE**
- Final product scope: operation-aware Build/Bulldoze/invalid Preview, release-outside-Terrain finalization, and screen-relative pan at rotated yaw.
EOF
pnpm exec prettier --write docs/superpowers/verification/2026-08-02-operation-aware-interaction.md

git diff --check
test ! -e .github/workflows/operation-aware-interaction-implementation.yml
test ! -e .github/workflows/operation-aware-interaction-implementation-v2.yml
test ! -e .github/workflows/operation-aware-interaction-v3.yml
test ! -e .github/scripts/operation-aware-v3.sh
test ! -e .github/operation-aware-v3-trigger

git config user.name 'github-actions[bot]'
git config user.email '41898282+github-actions[bot]@users.noreply.github.com'
git add -A
git commit -m "fix: make Road and camera interaction operation-aware [run ${GITHUB_RUN_ID}]"
git push origin HEAD:master
git rev-parse HEAD | tee final-head.txt
