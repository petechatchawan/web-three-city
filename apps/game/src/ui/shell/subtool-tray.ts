...
const trayTools: Readonly<Record<TrayCategory, ReadonlyArray<readonly [string, GameToolMode]>>> = {
  terrain: [['Raise', 'raise'], ['Lower', 'lower'], ['Flatten', 'flatten']],
  roads: [['Build Road', 'road-build'], ['Bulldoze Road', 'road-bulldoze']],
  zones: [
...
const toolIcons: Readonly<Record<GameToolMode, CityIconName>> = {
  navigate: 'navigate', raise: 'raise', lower: 'lower', flatten: 'flatten',
  'road-build': 'roads', 'road-bulldoze': 'remove',
  'zone-residential': 'residential', 'zone-commercial': 'commercial',
  'zone-industrial': 'industrial', 'zone-remove': 'remove',
...
export interface SubToolTray {
  ...
  readonly categories: Readonly<Record<TrayCategory, ReadonlyArray<readonly [string, GameToolMode]>>>;
  open(category: TrayCategory): void;
  close(): void;
  dispose(): void;
}

export function mountSubToolTray(parent: HTMLElement, callbacks: SubToolTrayCallbacks): SubToolTray {
