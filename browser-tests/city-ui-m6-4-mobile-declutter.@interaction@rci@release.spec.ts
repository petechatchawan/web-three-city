...
test('M6.4 exposes one Build entry and closes the picker after a concrete tool is chosen', async ({ page }) => {
  ...
  const contextHeight = await page.getByTestId('status-feedback').evaluate((element) =>
    element.getBoundingClientRect().height,
  );
  expect(contextHeight).toBeLessThanOrEqual(72);
});

test('M6.4 Inspect is a bounded contextual surface instead of a primary dialog', async ({ page }) => {
  await openMobile(page);
  ...
  const collapsedHeight = await inspect.evaluate((element) => element.getBoundingClientRect().height);
  ...
  const expandedHeight = await inspect.evaluate((element) => element.getBoundingClientRect().height);
  expect(expandedHeight).toBeLessThanOrEqual(896 * 0.45 + 1);
});

test('M6.4 renders explicit RCI demand bars and supports EN/TH without overflow', async ({ page }) => {
  await openMobile(page);
