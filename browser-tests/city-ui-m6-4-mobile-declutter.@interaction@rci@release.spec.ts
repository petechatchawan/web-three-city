...
test(
  'M6.4 exposes one Build entry and closes the picker after a concrete tool is chosen',
  async ({ page }) => {
...
test(
  'M6.4 Inspect is a bounded contextual surface instead of a primary dialog',
  async ({ page }) => {
    await openMobile(page);
...
test(
  'M6.4 renders explicit RCI demand bars and supports EN/TH without overflow',
  async ({ page }) => {
    await openMobile(page);
