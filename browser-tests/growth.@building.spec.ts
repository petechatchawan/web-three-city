...
test('automatic Growth preserves the active Zoning tool and in-progress stroke', async ({
  ...
  expect(snapshot.simulation.absoluteGameMinute).toBe(12 * 60);
  expect(snapshot.simulation.absoluteTick).toBe(12);
...
test('persists WorldSaveV8 and loads paused at the exact logical minute', async ({ page }) => {
  await openGrowthGame(page);
  await prepareBuildingFixtureWorld(page);
  ...
  expect(parsed.simulation).toMatchObject({ absoluteGameMinute: 12 * 60, growthSequence: 1 });
