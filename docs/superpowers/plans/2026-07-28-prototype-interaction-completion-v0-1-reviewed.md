...
**Architecture:** `packages/camera-input` owns camera state, projected framing, gesture arbitration, anchoring, and disposable browser bindings. `packages/terrain-three` owns selected-cell and grid overlays. `apps/game` composes those units and exposes read-only diagnostics for browser acceptance without mutating authoritative Terrain data.
...
```ts
const events: Array<{ type: string; point?: { x: number; y: number } }> = [];
...
Each test creates a fresh `events` array/controller in `beforeEach` so state does not leak.
...
- [ ] **Step 5: Implement exact frame metrics and stabilization**
...
- [ ] **Step 1: Write failing binding tests with complete fixture setup**
...
Both presentation classes expose read-only `object3d` for composition/tests.

```ts
it('toggles grid without replacing root geometry', () => {
  grid.load(snapshot);
...
### Task 6: Responsive Game composition and diagnostics
...
- Produces: interactive Game shell and shared browser-test helpers

- [ ] **Step 1: Create exact evidence contract and browser helper**
...
Render status, quality, Save, Load, Rotate left/right, Reset, Grid (`aria-pressed`), selected-cell text, and controls mode. Define CSS `--safe-top/right/bottom/left` using `env(safe-area-inset-*, 0px)`. Measure panel/canvas rects plus parsed safe-area properties. Desktop reserves panel right + `16px`; mobile `max-width:720px` reserves panel bottom + `8px`.
...
**Interfaces:**
- Consumes: `GAME_URL`, `readEvidence`, `dispatchTouch`
...
```ts
import { expect, test } from '@playwright/test';
import { GAME_URL, dispatchTouch, readEvidence } from './helpers/interaction.js';

test('pinch zooms without selection', async ({ page }) => {
  await page.goto(GAME_URL);
  const before = (await readEvidence(page)).camera.orthographicSize;
  await dispatchTouch(page, 'pointerdown', 1, 500, 400);
  await dispatchTouch(page, 'pointerdown', 2, 700, 400);
  await dispatchTouch(page, 'pointermove', 1, 480, 400);
  await dispatchTouch(page, 'pointermove', 2, 720, 400);
  await dispatchTouch(page, 'pointermove', 1, 460, 400);
  await dispatchTouch(page, 'pointermove', 2, 740, 400);
  await dispatchTouch(page, 'pointerup', 1, 460, 400);
  await dispatchTouch(page, 'pointerup', 2, 740, 400);
...
test('twist produces continuous yaw', async ({ page }) => {
  ...
  await dispatchTouch(page, 'pointerdown', 1, 500, 400);
  await dispatchTouch(page, 'pointerdown', 2, 700, 400);
  await dispatchTouch(page, 'pointermove', 1, 510, 380);
  await dispatchTouch(page, 'pointermove', 2, 690, 420);
  await dispatchTouch(page, 'pointermove', 1, 525, 365);
  await dispatchTouch(page, 'pointermove', 2, 675, 435);
...
test('parallel upward drag increases pitch within limits', async ({ page }) => {
  await page.goto(GAME_URL);
  await dispatchTouch(page, 'pointerdown', 1, 500, 450);
  await dispatchTouch(page, 'pointerdown', 2, 700, 450);
  await dispatchTouch(page, 'pointermove', 1, 500, 430);
  await dispatchTouch(page, 'pointermove', 2, 700, 430);
  await dispatchTouch(page, 'pointermove', 1, 500, 410);
  await dispatchTouch(page, 'pointermove', 2, 700, 410);
...
- [ ] **Step 2: Write third-contact and cancellation tests**
...
test('third contact suppresses until all contacts release', async ({ page }) => {
  ...
  await dispatchTouch(page, 'pointerdown', 1, 500, 400);
  await dispatchTouch(page, 'pointerdown', 2, 700, 400);
  await dispatchTouch(page, 'pointerdown', 3, 600, 500);
  await dispatchTouch(page, 'pointermove', 1, 450, 400);
  await dispatchTouch(page, 'pointerup', 3, 600, 500);
  await dispatchTouch(page, 'pointermove', 2, 750, 400);
  await dispatchTouch(page, 'pointerup', 1, 450, 400);
  await dispatchTouch(page, 'pointerup', 2, 750, 400);
...
test('pointer cancellation cannot select', async ({ page }) => {
  await page.goto(GAME_URL);
  await dispatchTouch(page, 'pointerdown', 1, 500, 400);
  await dispatchTouch(page, 'pointercancel', 1, 500, 400);
  const evidence = await readEvidence(page);
  expect(evidence.selectedCell).toBeNull();
  expect(evidence.activePointerCount).toBe(0);
});
...
test('a pointer starting on UI never moves the world', async ({ page }) => {
  ...
  const box = await page.getByRole('button', { name: 'Save terrain' }).boundingBox();
  if (box === null) throw new Error('missing Save terrain bounds');
  await dispatchTouch(page, 'pointerdown', 1, box.x + 5, box.y + 5);
  await dispatchTouch(page, 'pointermove', 1, 900, 500);
  await dispatchTouch(page, 'pointerup', 1, 900, 500);
  const after = await readEvidence(page);
...
test('context loss clears an active session before release', async ({ page }) => {
  ...
  await dispatchTouch(page, 'pointerup', 1, 500, 400);
  const evidence = await readEvidence(page);
  expect(evidence.activePointerCount).toBe(0);
  expect(evidence.selectedCell).toBeNull();
});
