...
function addBox(group: THREE.Group, size: readonly [number, number, number], position: readonly [number, number, number], material: THREE.Material, name: string): void {
...
export function createBuildingPrototype(instance: BuildingInstance, materials: BuildingMaterials, config: WorldConfig): THREE.Group {
  ...
  switch (definition.prototypeId) {
    ...
    case 'rowhouse':
      ...
      break;
    case 'duplex':
      addBox(group, [0.78, h * 0.72, 0.82], [-0.42, h * 0.36, 0], bodyMaterial, 'building-body-left');
      addBox(group, [0.78, h * 0.72, 0.82], [0.42, h * 0.36, 0], bodyMaterial, 'building-body-right');
    ...
    case 'apartment':
      ...
      for (const y of [0.45, 0.95, 1.45, 1.95]) addBox(group, [1.7, 0.05, 1.7], [0, y, 0], materials.accent, 'building-window-band');
    ...
    case 'factory':
      ...
      for (const x of [-0.55, 0, 0.55]) addBox(group, [0.5, 0.2, 1.78], [x, h * 0.76, 0], materials.roof, 'building-stepped-roof');
      addBox(group, [0.22, h * 0.75, 0.22], [0.55, h * 0.8, -0.5], materials.accent, 'building-chimney');
