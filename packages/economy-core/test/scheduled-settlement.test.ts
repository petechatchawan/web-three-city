...
describe('scheduled municipal settlement', () => {
  it('does not settle repeatedly during minute transitions within 08:00', () => {
    const result = economy.settleScheduledEconomy(
      initial(),
      {
        ...
        calendar: { year: 1, month: 1, hour: 8 },
        ...projections,
      },
      rules,
    );
  ...
  it('keeps the 08:00 settlement cadence at one settlement per 24-hour cycle', () => {
    const afterEight = economy.settleScheduledEconomy(
      initial(8),
      {
        ...
        calendar: { year: 1, month: 1, hour: 9 },
        ...projections,
      },
      rules,
    );
    ...
    const nextCycle = economy.settleScheduledEconomy(
      initial(8),
      {
        ...
        calendar: { year: 1, month: 2, hour: 8 },
        ...projections,
      },
      rules,
    );
  ...
  it('closes the old month before placing Day 1 revenue into the new period', () => {
    ...
    const nextMonth = economy.settleScheduledEconomy(
      seeded.snapshot,
      {
        beforeMacroHourIndex: macroHourIndex(55),
        afterMacroHourIndex: macroHourIndex(56),
        calendar: { year: 1, month: 3, hour: 8 },
        ...projections,
      },
      rules,
    );
    expect(nextMonth).toMatchObject({
      ...
      status: 'settled',
      snapshot: {
        currentPeriod: { year: 1, month: 3, taxRevenue: { residentialMinor: 2_100 } },
        previousPeriod: { year: 1, month: 2, taxRevenue: { residentialMinor: 2_100 } },
        lastMonthlyCloseAtMacroHourIndex: macroHourIndex(56),
      },
    });
  });
});
