...
function advanceOneQuantum(
  ...
  if (trip.mode === 'Drive' && trip.activeNodeTraversal !== undefined) {
    const progressQ = Math.min(
      TRAFFIC_PROGRESS_MAX_Q,
      trip.activeNodeTraversal.progressQ + 250_000,
    );
    if (progressQ < NODE_TRAVERSAL_REAR_CLEARANCE_PROGRESS_Q) {
      return Object.freeze({
        trip: Object.freeze({
          ...trip,
          activeNodeTraversal: Object.freeze({ ...trip.activeNodeTraversal, progressQ }),
        }),
        arrived: false,
        newlyQueued: false,
        reservations,
    ...
    return Object.freeze({
      trip: Object.freeze({
        ...
        progressQ: 0,
        lastStableNodeId: trip.activeNodeTraversal.nodeId,
      }),
      arrived: false,
      newlyQueued: false,
      reservations: releaseTrafficReservationBundle({
        ledger: reservations,
        tripId: trip.tripId,
        resourceIds: trip.activeNodeTraversal.reservedResourceIds,
      }),
    });
  }
  if (trip.mode === 'Drive') {
