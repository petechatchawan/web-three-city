...
import type { TerraformPlan } from "@web-three-city/terraform";
import * as composition from "@web-three-city/terraform/composition";

interface TerraformUndoEntryUnderTest {
...
interface TerraformUndoHistoryUnderTest {
  depth(): number;
  expectedTerrainRevision(): TerrainRevision;
  recordCommit(plan: TerraformPlan, newRevision: TerrainRevision): void;
  peekUndo(currentRevision: TerrainRevision): TerraformUndoEntryUnderTest | undefined;
  recordUndo(newRevision: TerrainRevision): void;
  synchronizeExternalRevision(currentRevision: TerrainRevision): void;
  clear(): void;
}

type TerraformCompositionUnderTest = typeof composition & {
  readonly createTerraformUndoHistory?: (
    initialRevision: TerrainRevision,
  ) => TerraformUndoHistoryUnderTest;
};

const api = composition as TerraformCompositionUnderTest;
...
function create(initialRevision = 100): TerraformUndoHistoryUnderTest | undefined {
  return api.createTerraformUndoHistory?.(revision(initialRevision));
}
