...
const PRODUCTION_MIN_ELEVATION = 32;
const PRODUCTION_MAX_ELEVATION = 288;
...
describe("production Terrain generation", () => {
  ...
  it("keeps all production elevations inside the frozen envelope", () => {
    const field = generateProductionTerrainField(PRODUCTION_SEED);
    let count = 0;
    let min = Number.POSITIVE_INFINITY;
    let max = Number.NEGATIVE_INFINITY;
    let firstOutOfEnvelope:
      | { readonly x: number; readonly z: number; readonly elevation: number }
      | undefined;
    ...
    for (let z = 0; z < field.vertexHeight; z += 1) {
      for (let x = 0; x < field.vertexWidth; x += 1) {
        ...
        if (
          firstOutOfEnvelope === undefined &&
          (elevation < PRODUCTION_MIN_ELEVATION ||
            elevation > PRODUCTION_MAX_ELEVATION)
        ) {
          firstOutOfEnvelope = { x, z, elevation };
        }
      }
    }

    expect(firstOutOfEnvelope).toBeUndefined();
    expect(count).toBe(field.vertexWidth * field.vertexHeight);
    expect(min).toBeGreaterThanOrEqual(PRODUCTION_MIN_ELEVATION);
    expect(max).toBeLessThanOrEqual(PRODUCTION_MAX_ELEVATION);
  });
