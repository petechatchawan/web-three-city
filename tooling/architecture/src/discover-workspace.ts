...
async function readManifest(packageRoot: string, relativeRoot: string): Promise<WorkspacePackage> {
  const raw = JSON.parse(await readFile(path.join(packageRoot, 'package.json'), 'utf8')) as unknown;
  if (!isRecord(raw) || typeof raw.name !== 'string') {
    throw new Error(`Workspace package ${relativeRoot} must declare a string package name.`);
  }
  return {
    name: raw.name,
    root: packageRoot,
...
async function rootsForPattern(root: string, pattern: string): Promise<string[]> {
  const normalized = pattern.replaceAll('\\', '/').replace(/^\.\//, '');
  if (!normalized.includes('*')) {
    return (await exists(path.join(root, normalized, 'package.json'))) ? [normalized] : [];
  }
  if (!normalized.endsWith('/*') || normalized.slice(0, -2).includes('*')) {
    throw new Error(`Unsupported workspace pattern: ${pattern}. Bootstrap architecture expects one-level namespace globs.`);
  }
  ...
  for (const entry of entries) {
    ...
    if (await exists(path.join(root, relativeRoot, 'package.json'))) roots.push(relativeRoot);
  }
  return roots.sort();
}

export async function discoverWorkspace(root: string): Promise<WorkspaceDiscovery> {
  ...
  const patterns = document.packages.filter((value): value is string => typeof value === 'string');
  const relativeRoots = new Set<string>();
  for (const pattern of patterns) {
    for (const relativeRoot of await rootsForPattern(root, pattern)) relativeRoots.add(relativeRoot);
  }
  const packages: WorkspacePackage[] = [];
  for (const relativeRoot of [...relativeRoots].sort()) {
    packages.push(await readManifest(path.join(root, relativeRoot), relativeRoot));
  }
