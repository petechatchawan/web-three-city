...
export async function checkArchitecture(repositoryRoot: string): Promise<ArchitectureReport> {
  ...
  const importResult = checkImportRules(discovery.packages, analysis.imports, policy);
  ...
  return {
    packages: discovery.packages
      .map((pkg) => ({ name: pkg.name, path: pkg.relativeRoot, profile: pkg.profile }))
      .sort((a, b) => a.path.localeCompare(b.path)),
    edges: [...importResult.edges].sort((a, b) => `${a.from}\0${a.to}\0${a.surface}\0${a.sourceFile}`.localeCompare(`${b.from}\0${b.to}\0${b.surface}\0${b.sourceFile}`)),
