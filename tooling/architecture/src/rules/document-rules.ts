...
function violation(ruleId: string, source: string, message: string, reference: string): ArchitectureViolation {
...
export async function checkDocumentRules(root: string): Promise<ArchitectureViolation[]> {
  ...
  for (const file of await markdownFiles(path.join(root, 'docs', 'architecture'))) {
    ...
    if (!statusMatch) {
      violations.push(violation('ARCH-DOC-001', file, 'Architecture document is missing the required explicit status header.', 'A10 § Document Status Model'));
      continue;
    }
