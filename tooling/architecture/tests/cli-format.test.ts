...
const cleanReport: ArchitectureReport = {
  packages: [{ name: '@web-three-city/tooling-architecture', path: 'tooling/architecture', profile: 'tooling' }],
  edges: [],
  violations: []
};
...
describe('architecture CLI formatting', () => {
  ...
  test('formats clean reports with an explicit summary', () => {
    expect(formatReport(cleanReport)).toBe('Architecture check passed: 1 package, 0 edges, 0 violations.');
  });
  ...
  test('formats violations with stable rule, source, target, reason, and binding reference', () => {
    ...
    expect(output).toContain('[ARCH-DEMO-001] systems/alpha/src/index.ts -> @web-three-city/beta');
