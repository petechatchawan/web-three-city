...
function extractBrowserTags(file) {
  ...
  return sorted(
    tags.map((tag) => tag.toLowerCase()).filter((tag) => Object.hasOwn(BROWSER_TAG_SYSTEMS, tag)),
  );
}

function browserSystems(tags) {
  return sorted(
    tags.flatMap((tag) => (BROWSER_TAG_SYSTEMS[tag] ? [BROWSER_TAG_SYSTEMS[tag]] : [])),
  );
}

function isBrowserSpec(file) {
  return (
    normalizePath(file).startsWith('browser-tests/') &&
    /\.(?:spec|test)\.[cm]?[jt]sx?$/.test(normalizePath(file))
  );
}
...
function isDeterministicTest(file) {
  const normalized = normalizePath(file);
  return (
    /(?:^|\/)(?:test|tests)\//.test(normalized) || /\.(?:test|spec)\.[cm]?[jt]sx?$/.test(normalized)
  );
}
...
function ownerClassification(file, authority) {
  ...
  const gameRule =
    authority === VerificationAuthority.PRODUCT_SOURCE ? gameSourceRuleForFile(file) : null;
  return {
...
export function classifyChangedFile(file) {
    ...
    return {
      ...
      reason:
        browserTags.length > 0
          ? 'approved browser ownership tags select targeted evidence'
          : 'browser contract requires browser evidence',
    };
  }
  ...
  if (isDeterministicTest(normalized)) {
    ...
    return {
      ...
      deploymentRequired: false,
      reason:
        owner.systems.length > 0
          ? 'deterministic test authority is below the browser'
          : 'deterministic test has no known owner',
    };
  }
